...
def openai_api_call(chat_messages: list[dict], model_name: str = "gpt-4", temperature: float = 0.0, max_tokens: int = 1) -> str:
    ...
    response = OPENAPI_CLIENT.chat.completions.create(**args)
...
def get_response(query: str, config: dict):
    ...
    thread_id = superpowered.create_chat_thread([kb_id], use_rse=use_rse, system_message=system_message)["id"] # create a chat thread with the provided config parameters
    chat_response = superpowered.get_chat_response(thread_id, query)
...
def run_evaluation(eval_set: list[dict], config: dict):
    ...
    with open(f"{DIR}/eval_results.json", "w") as f:
